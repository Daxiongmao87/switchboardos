...
class FakeSshFileService {
  ...
  async delete(input) {
    ...
    return {
      ...
      command: input.recursive ? 'delete recursive' : 'delete',
      stdout: SECRET_MARKER,
      stderr: '',
      exitCode: 0,
      durationMs: 5,
...
async function main() {
  ...
  try {
    ...
    const deniedDelete = await jsonRequest(baseUrl, '/api/ssh-files/delete', {
      method: 'POST',
      cookie,
      csrfToken,
      body: { hostId: HOST_ID, path: '/tmp/delete-target.txt' },
    });
    ...
    const deleteResult = await jsonRequest(baseUrl, '/api/ssh-files/delete', {
      method: 'POST',
      cookie,
      csrfToken,
      body: { hostId: HOST_ID, path: '/tmp/delete-target.txt' },
    });
    assert.equal(deleteResult.status, 200, 'balanced policy allows SSH file delete through hosted API');
    ...
    assert.equal(deleteAudit.metadata.remotePathLogged, false);
    assert.equal(deleteAudit.metadata.commandTextLogged, false);
